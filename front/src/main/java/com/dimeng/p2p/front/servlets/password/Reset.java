...
public class Reset extends AbstractPasswordServlet {
	...
	private final static Logger log = Logger.getLogger(Reset.class);
	...
	protected void processPost(HttpServletRequest request, HttpServletResponse response, ServiceSession serviceSession) throws Throwable {
		...
		/**
		 * Client uc = new Client(); String accunname =
		 * passwordManage.getaccoutName(id); String accunname =
		 * passwordManage.getaccoutName(id); if(accunname!=null &&
		 * !("").equals(accunname)){ try{ String
		 * passwordp=UnixCrypt.crypt(password,DigestUtils.sha256Hex(password));
		 * String rs=uc.uc_user_edit(accunname, passwordp,passwordp, "", 1, "",
		 * ""); if(rs!=null && !"".equals(rs)){ int prs=Integer.parseInt(rs);
		 * if(prs==1 || prs==0){ System.out.println(accunname+"论坛同步修改成功");
		 * }else{ System.out.println(accunname+"论坛同步修改不成功"); }}else{
		 * System.out.println(accunname+"论坛同步修改不成功"); }}catch(Exception e){
		 * 
		 * } }
		 **/
