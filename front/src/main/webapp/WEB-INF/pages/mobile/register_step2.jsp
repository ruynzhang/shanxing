...
<head>
  ...
  <script type="text/javascript" src="<%=controller.getStaticPath(request)%>/js/register.js"></script>
   <script src="<%=controller.getStaticPath(request)%>/phone/js/common.js" type="text/javascript" ></script>
    <style>body{background-color: #f7f8f8}.sx_popup .sx_err_tip{padding-top:0;margin-top: 60px;} .sx_m_showbox{height:400px;overflow-y: scroll;overflow-x: hidden;}
...
<div class="wx_login_box">
    <form action="<%=configureProvider.format(URLVariable.MWEB_REGISTERSTEP2) %>" method="post" id="form1" onsubmit="return phoneVerifyCodeCheck()">
       <%String mobilephone = serviceSession.getSession().getAttribute("register_step1_mobilephone"); %>
        <p class="sj_num">您的手机号码是: <b><%=mobilephone == null ? "" : mobilephone%></b> 点击获取验证码</p>
        <div class="wx_login_check_code input_box">
            <input type="text" name="phoneVerifyCode" maxlength="6" placeholder="请输入验证码" value="<%StringHelper.filterQuoter(out, request.getParameter("phoneVerifyCode"));%>" onblur="phoneVerifyCodeCheck()"   class="icon_mb input_text" >
            <input type="hidden" name="mobilephone" value="<%=mobilephone == null ? "" : mobilephone%>"/>
            <span class="vcode mb_code"><input type="button" value="获取验证码" onclick="getVerifyCode(this)"></span>
        </div>

        <a href="javascript:void(0);"  onclick="resubmit();"  class="btn_wap wx_btn_orange"> 下一步</a>
...
<div class="sx_popup" style="display: block;">
   ...
   <p><a href="javascript:void(0);" onclick="subdisty();" class="sx_err_btn">确定</a></p>
</div>
<%}}%>
