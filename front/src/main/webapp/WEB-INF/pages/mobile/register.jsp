...
<style>
    ...
    .sx_m_showbox{height:400px;overflow-y: scroll;overflow-x: hidden;}
    .sx_m_showbox p{padding:40px 10px 15px 10px}
    .sx_m_close{  width: 32px;
    height: 32px;
    background: #3E8CE1;
    color: #fff;
    line-height: 32px;
    ...
    .sj_m_close{ margin: 0 auto;}
    </style>
</head>
<body>
<%if(dimengSession!=null&&dimengSession.isAuthenticated()){controller.sendRedirect(request, response,configureProvider.format(URLVariable.MWEB_LIST));return;} %>
...
<div class="wx_login_box">
    <form action="<%=configureProvider.format(URLVariable.MWEB_REGISTER) %>" id="form1" method="post">
        ...
        <%}}%>
        <div style="display:none" class="input_box">
           <span class="user_icon"></span>
        	<input type="hidden" name="partId" id="partId" value="<%StringHelper.filterQuoter(out, request.getParameter("partId"));%>"/>
            <input type="text"  placeholder="用户名"  name="accountName" value="" class="input_text" >
        </div>
        
        <div class="input_box">
           <span class="mobile_icon"></span>
            <input type="text" name="mobilephone" value="<%StringHelper.filterQuoter(out, request.getParameter("mobilephone"));%>"  placeholder="手机号码"   class="icon_mb input_text" >
        </div>
        <div class="input_box">
           <span class="passwd_icon"></span>
            <input type="password"  placeholder="密码"  name="password" class=" input_text" >
        </div>
        <div class="input_box" id="codeDiv">
           <span class="tjm_icon"></span>
            <input type="text"  placeholder="邀请码，可以不填写" value="<%StringHelper.filterQuoter(out, request.getParameter("code"));%>"   name="code" class="input_text" >
        </div>
        ...
        <div class="wx_login_check_code">
            ...
            <span class="sj_code"> <img id="codeNum" src="<%=controller.getURI(request, RegisterVerify.class)%>" class="verifyImg" onclick='this.src="<%=controller.getURI(request, RegisterVerify.class)%>?"+Math.random()' style="width:109px;height:43px;" title="换一张" alt="验证码"> </span>
        </div>
        ...
        <div class="xn_login_blow">
            <span class="xn_noAccount">已阅读 并同意<a class="showText sx_blueColor"> 善行创投注册协议 </a> </span>
        </div>
    </form>
</div>
...
本公司对本服务协议享有最终的解释权。</p>
<input type="button" class="index-btn sj_m_close" value="确定">
</div>
<div class="clear"></div>
...
var shareData = {
  title: '善行创投',
  desc: '呼朋唤友抢红包，注册立送50元，善行创投-安全家庭理财进行中',
  link: 'http://www.myshanxing.com/pay/slaward.html<%=request.getParameter("code") == null?"":"?code="+request.getParameter("code")%>',
