...
@Right(id = "P2P_C_SPREAD_TGXQ_SEARCH", isMenu=true, name = "添加业务员信息")
public class Add extends AbstractSpreadServlet{
	...
	protected void processPost(final HttpServletRequest request,
		...
		try{
			String ywyId= manage.searchTel(request.getParameter("tel"));
			if(ywyId != null && !ywyId.isEmpty()){
				throw new ParameterException("手机号已存在");
			}
			
			
			manage.add(new SalesmanQuery() {
				@Override
				public String ywyId() {
					return "";
				}
	...
	protected void onThrowable(HttpServletRequest request,
		...
		if (throwable instanceof ParameterException
			...
			sendRedirect(request, response, getController().getViewURI(request, AddSalesman.class));
		}  else {
