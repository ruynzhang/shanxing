...
@Right(id = "P2P_C_FINANCE_ADDSXBAO", name = "添加善行宝信息")
public class AddSxbao extends AbstractBidServlet{
	...
	protected void processPost(HttpServletRequest request, HttpServletResponse response, ServiceSession serviceSession) throws Throwable {
		...
		try {
			...
			//判断标题是否存在
			if(sxbaoManage.isExistTitle(sxbaoConfig.title)){
			...
			sxbaoManage.addConfig(sxbaoConfig);
