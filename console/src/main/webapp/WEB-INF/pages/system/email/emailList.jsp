...
<%@page import="com.dimeng.p2p.console.servlets.system.email.ViewEmail"%>
...
<body>
	...
	<div class="w_main">
		<div class="main clearfix">
			<div class="wrap">
				<div class="r_main">
					<form action="<%=controller.getURI(request, EmailList.class)%>" method="post">
						<div class="home_main">
						...
						request.getParameter("createTimeEnd"));%>" />
					                   ...
					                   <div class="mt10">
									    ...
									    %>
										 <a href="<%=controller.getURI(request, AddEmail.class)%>" class="btn3 mr10"><span class="ico1"></span>添加</a>
										<%}else{ %>
							...
							<div>
								<table width="100%" border="0" cellspacing="0" cellpadding="3" class="yhgl_table tc">
									...
									<tr class="dhsbg">
										...
										<td title="<%StringHelper.filterHTML(out, email.content);%>">
											<%StringHelper.filterHTML(out, StringHelper.truncation(email.content, 15));%>
										</td>
										<td><%
										...
										<td><%=email.count%></td>
										<td><%=DateTimeParser.format(email.createTime)%></td>
										<td><span>
										<%
											if(dimengSession.isAccessableResource(ViewEmail.class))
											{
											%>
										<a href="<%=controller.getURI(request, ViewEmail.class)%>?id=<%=email.id %>" class="mr10 blue">查看</a>
										<%}else{ %>
										<span>查看</span>
										<%} %>
										</span></td>
