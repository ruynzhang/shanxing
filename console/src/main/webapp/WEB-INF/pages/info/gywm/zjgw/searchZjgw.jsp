...
<div class="main clearfix">
	 ...
	 %>
	 <form action="<%=controller.getURI(request, SearchZjgw.class)%>" method="post">
  	 <div class="wrap">
        <div class="r_main">
            <div class="home_main">
            	<div class="box box1 mb15">
                    ...
                    <p>
                    <span>姓名：</span><span><input type="text" name="title" id="textfield" class="yhgl_input" value="<%StringHelper.filterHTML(out, request.getParameter("title"));%>"/></span>
                    </p>
                    <p>
                    ...
                    <%if (dimengSession.isAccessableResource(AddZjgw.class)) {%>
                    	<a href="<%=controller.getURI(request, AddZjgw.class)%>" class="btn3 mr10"><span class="ico3"></span>添加</a>
                    <%}else{ %>
                  ...
                  <table width="100%" border="0" cellspacing="0" cellpadding="3" class="yhgl_table tc">
                        ...
                        <tr class="dhsbg">
                          ...
                          <%if (dimengSession.isAccessableResource(UpdateZjgw.class)) {%>
                          	<a href="<%=controller.getURI(request, UpdateZjgw.class)%>?id=<%=record.id %>">修改</a>
                          <%}else{ %>
							<span class="gray6">修改</span>
						 <%} %> 
						<%if (dimengSession.isAccessableResource(DeleteZjgw.class)) {%> 
                          	<a href="<%=controller.getURI(request, DeleteZjgw.class)%>?id=<%=record.id %>">删除</a>
                         <%}else{ %>
