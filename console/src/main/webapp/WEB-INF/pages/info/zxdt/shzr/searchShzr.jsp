...
<div class="main clearfix">
	 ...
	 <form action="<%=controller.getURI(request, SearchShzr.class)%>" method="post">
  	 <div class="wrap">
        <div class="r_main">
            <div class="home_main">
            	<div class="box box1 mb15">
                    ...
                    <p>
                    <span>标题：</span><span><input type="text" name="title" id="textfield" class="yhgl_input" value="<%StringHelper.filterHTML(out, request.getParameter("title"));%>"/></span>
                    </p>
                    <p>
                    ...
                    <span>发布状态：</span>
                    <span>
                    	<select name="status"  class="yhgl_input">
                    		<option >全部</option>
                    ...
                    <%if (dimengSession.isAccessableResource(AddShzr.class)) {%>
                    	<a href="<%=controller.getURI(request, AddShzr.class)%>" class="btn3 mr10"><span class="ico3"></span>添加</a>
                    <%}else{ %>
                  ...
                  <table width="100%" border="0" cellspacing="0" cellpadding="3" class="yhgl_table tc">
                        ...
                        <tr class="dhsbg">
                          ...
                          <%if (dimengSession.isAccessableResource(UpdateShzr.class)) {%>
                          	<a href="<%=controller.getURI(request, UpdateShzr.class)%>?id=<%=record.id %>">修改</a>
                          <%}else{ %>
							<span class="gray6">修改</span>
						<%} %> 
						<%if (dimengSession.isAccessableResource(DeleteShzr.class)) {%>  
                          	<a href="<%=controller.getURI(request, DeleteShzr.class)%>?id=<%=record.id %>">删除</a>
                         <%}else{ %>
