...
<%
		...
		PagingResult<ExpemoneyGame> result = (PagingResult<ExpemoneyGame>) request
				.getAttribute("result");
%>
<%
		CURRENT_CATEGORY = "YWGL";
			CURRENT_SUB_CATEGORY = "YXDH";
	%>
<body>
	...
	<div class="w_main">
		<div class="main clearfix">
			<div class="wrap">
				<div class="r_main">
					<div class="home_main">
						<form action="<%controller.getURI(request, GamedhList.class);%>" method="post" name="form1" id="searchForm">
							<div class="box box1 mb15">
								...
								<div class="con">
									<div class="clear"></div>
									<ul class="yhxq_li">
										<li>兑换人数：<span class="blue"><%=request.getAttribute("ernum") %></span>
										</li>
									</ul>
									<ul class="yhxq_li">
										<li>未兑换人数 ：<span class="blue"><%=request.getAttribute("nernum") %></span>
										</li>
									</ul>
									<ul class="yhxq_li">
										<li>兑换总额 ：<span class="blue"><%=request.getAttribute("eramount") %></span>
										</li>
									</ul>
									<ul class="yhxq_li">
										<li>未兑换总额 ：<span class="blue"><%=request.getAttribute("ernoamount") %></span>
										</li>
									</ul>
									<ul class="yhxq_li">
										<li>被邀人数 ：<span class="blue"><%=request.getAttribute("invinum") %></span>
										</li>
									</ul>
									<ul class="yhxq_li">
										<li>实名人数 ：<span class="blue"><%=request.getAttribute("relgamename") %></span>
										</li>
									</ul>
									<div class="clear"></div>
								</div>
								...
								<div class="con">
									<div class="admin_input admin_pb">
             							<div class="table top3">
											...
											<p>
												<span>被邀姓名：</span>
												<input type="text" name="inviteName" value="<%StringHelper.filterHTML(out, request.getParameter("inviteName")); %>" class="yhgl_input" />
											</p>
											<P>
												<span>是否兑换：</span>
											...
											<P>
												...
												<%if (dimengSession.isAccessableResource(GamedhListExport.class)) {%>
							...
							<table width="100%" border="0" cellspacing="0" cellpadding="3" class="yhgl_table tc">
								...
								<%
								ExpemoneyGame[] items = result.getItems();
										if (items != null) {
											int i=1;
											for (ExpemoneyGame exgame: items) {
												if (exgame == null) {
													continue;
												}
								%>
								<tr class="dhsbg">
									...
									<td>
									   <%=DateTimeParser.format(exgame.regitime)
									   %>
									</td>
									<td><%=DateTimeParser.format(exgame.gettime)
									   %></td>
									<td><%if(exgame.gettime != null)%><%=Formater.formatAmount(exgame.money)%> </td>
									<td><%if(exgame.gettime == null)%><%=Formater.formatAmount(exgame.money)%> </td>
									<td><%StringHelper.filterHTML(out,exgame.invitename);%></td>
								</tr>
								<%
									}
										}
							...
							<%
							    GamedhList.rendPagingResult(out, result);
							%>
	...
	function showExport()
	{
		document.getElementById("searchForm").action ="<%=controller.getURI(request, GamedhListExport.class)%>";
		$("#searchForm").submit();
		document.getElementById("searchForm").action ="<%=controller.getURI(request, GamedhList.class)%>";
	}
	$(function() {
