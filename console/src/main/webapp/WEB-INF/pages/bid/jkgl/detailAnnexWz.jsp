...
<%@page import="com.dimeng.p2p.console.servlets.bid.jkgl.SetViewPic"%>
...
<%@page import="com.dimeng.p2p.console.servlets.bid.jkgl.NotThrough"%>
<%@page import="com.dimeng.p2p.console.servlets.bid.jkgl.Check"%>
<%@page import="com.dimeng.p2p.console.servlets.bid.jkgl.AdjustCredit"%>
							...
							<div class="border p30 Men_bt no_table">
								...
								%>
									<div class="tc w350 pt20">
										...
										<a href="javascript:void(0)" onclick="showSh()" class="btn5">审核不通过</a>
										<%}else{ %>
										<a href="javascript:void(0)" class="btn5 mr10">审核通过</a>
										<a href="javascript:void(0)" onclick="showSh()" class="btn4 mr10">审核不通过</a>
										<%} %>
									</div>
			                    	<%if(creditAmount.compareTo(t6230.F05)<0){%>
			                    	<div><span style="color: red">用户信用额度小于借款金额,<a href="<%=controller.getURI(request, XyList.class)%>">调整</a></span></div>
			                    	<%} %>
			                    	<%if(DbCreditAmount.compareTo(t6230.F05)<0&&t6230.F11==T6230_F11.S){%>
			                    	<div> <span style="color: red">机构信用额度小于借款金额,<a href="<%=controller.getURI(request, XyList.class)%>">调整</a></span></div>
			                    	<%} %>
			                    <%} %>
								<div class="clear"></div>
							</div>
