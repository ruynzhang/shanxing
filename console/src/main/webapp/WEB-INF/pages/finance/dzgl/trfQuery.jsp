...
<%@page import="com.dimeng.p2p.escrow.huifu.entity.TrfItem"%>
...
<body>
	...
	<%
		...
		CURRENT_SUB_CATEGORY = "SHKKDZ";
		
		PagingResult<TrfItem> pagingResult = (PagingResult<TrfItem>)request.getAttribute("pagingResult");
	%>
	<div class="w_main">
					...
					<div class="home_main">
						<form action="<%controller.getURI(request, ChargeQuery.class);%>" method="post" name="form1" id="searchForm">
							...
							<table width="100%" border="0" cellspacing="0" cellpadding="3" class="yhgl_table tc">
								...
								<%
									TrfItem[] items = pagingResult.getItems();
									if(items != null){
										for(TrfItem item : items){
								%>
								<tr>
									<td><%=item.OrdId %></td>
									<td><%=item.PnrDate %></td>
									<td><%="S".equals(item.TransStat)?"成功":"失败" %></td>
									<td><%=item.TransAmt %></td>
