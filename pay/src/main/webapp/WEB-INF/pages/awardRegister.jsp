<%@page import="com.dimeng.p2p.pay.servlets.SendMobileVerifyCode"%>
<%@page import="com.dimeng.util.ObjectHelper"%>
<%@page import="com.dimeng.p2p.pay.servlets.CheckNameExists" %>
...
function mgstga(){
	var mcode =$("#mcode").val();
	var register = '<%configureProvider.format(out,URLVariable.REGISTER);%>';
	if(mcode==null || mcode=='' || mcode=='null'){
		 window.location.href = register;
	}else{
		 window.location.href = register + "?code="+mcode;
...
<script type="text/javascript">
	 ...
	 <%}}%>
	//var 判断手机号码是否重复;
	function summodeCodeabc(){
		   $.ajax({
			   type:"get", //请求方式
			   url:$("#regiajx").val(),//发送请求地址
			   data:{ 
			     phone:$("#phone").val()
			   },
...
<input type="hidden" id="regiajx" name="regiajx" value="<%=controller.getURI(request, SendMobileVerifyCode.class)%>"/>
<div id="sx_m_header">
    <div class="sx_m_logo"><a href="<%configureProvider.format(out,URLVariable.USER_AWARD);%>"><img src="<%=controller.getStaticPath(request)%>/mobile/images/white_logo.png" alt="" width="198"/></a></div><div class="sx_m_btBox">100%的中奖率，你还在等什么！！！</div>
</div>
<div class="sx_award_menu">
   ...
   <div class="sx_a_menubox"><a href="#" onclick="mgstga()" class="current">注册赢大奖</a><a href="<%configureProvider.format(out,URLVariable.LOGIN);%>">登陆再抽奖</a><a href="<%=controller.getURI(request, Activity.class) %>">抽奖</a></div>
</div>
<div id="sx_m_main">
     <!--block1-->
    <div class="sx_m_block1">
        <% //String mcode =request.getParameter("code");
        
        UserManage userManage =serviceSession.getService(UserManage.class);
         String mcode = userManage.getUserNameByCode(request.getParameter("code"));
          if(mcode!=null && !("").equals(mcode)){
         %>
        <div class="sx_m_text"><p><b class="sx_redColor"><%=mcode %></b><span class="sx_blueColor">伙伴邀请您注册加入善行创投，<b class="sx_redColor">100%</b>的中奖率，有钱就是这么任性！</span><br />
活动很简单，只要您注册，就有机会拿到我们的丰富大奖哦，赶紧往下瞄:<br />
      <% }else{ 
    	  mcode = "";
    	%>
          <div class="sx_m_text"><p>欢迎加入善行创投，<b class="sx_redColor">100%</b>的中奖率，有钱就是这么任性！</span><br />
活动很简单，只要您注册，就有机会拿到我们的丰富大奖哦，赶紧往下瞄:<br />
      <%} %>
       <p class="sx_tc">喵...........................我的喵啊!</p>
</div>
      <div class="container">
         <input type="hidden" id="mcode" value="<%=request.getParameter("code") %>"/>
	     <a class="toReg2"><img id="redux66" src="<%=controller.getStaticPath(request)%>/mobile/images/awardweb.jpg" alt="" class="get_award"/></a>
        </div>
      <div class="sx_m_bt1"><a href="#" onclick="mgstga();" class="toReg">戳我注册</a></div>
      <div class="sx_m_bt1 sx_m_bt2"><a href="<%configureProvider.format(out,URLVariable.LOGIN);%>" class="toLogin">已有帐号？立即登陆</a></div>
    </div>
    
     <!--block2-->
    <div class="sx_m_block2">
        <div class="sx_m_person1"></div>
        <ul class="sx_award_list">
          <li>
             <div class="sx_award_p1"></div>
             <div class="sx_award"><img src="<%=controller.getStaticPath(request)%>/mobile/images/m_aw1.png" alt="" /></div>
             <div class="sx_award_text"><h1>一等奖</h1>
             <p>你没看错，这就是传说中的<span class="sx_orangeColor">ipad Air2</span></p></div>
             <div class="clear"></div>
             
          </li>
          <li>
             <div class="sx_award"><img src="<%=controller.getStaticPath(request)%>/mobile/images/m_aw2.png" alt="" /></div>
             <div class="sx_award_text"><h1>二等奖</h1>
             <p>呜呜.......这是<span class="sx_orangeColor">三星GALAXY 平板电脑</span></p></div>
             <div class="clear"></div>
             <div class="sx_award_p2"></div>
          </li>
          <li>
             <div class="sx_award"><img src="<%=controller.getStaticPath(request)%>/mobile/images/m_aw3.png" alt="" /></div>
             <div class="sx_award_text"><h1>三等奖</h1>
             <p>咦....这不是我室友整天当宝的<span class="sx_orangeColor">亚马逊kindle</span></p></div>
             <div class="clear"></div>
             <div class="sx_award_p3"></div>
          </li>
          <li>
             <div class="sx_award"><img src="<%=controller.getStaticPath(request)%>/mobile/images/m_aw4.png" alt="" /></div>
             <div class="sx_award_text"><h1>四等奖</h1>
             <p><span class="sx_orangeColor">自动充值到您账户</span></p></div>
             <div class="clear"></div>
             <div class="sx_award_p4"></div>
          </li>
          <li>
             <div class="sx_award"><img src="<%=controller.getStaticPath(request)%>/mobile/images/m_aw5.png" alt="" /></div>
             <div class="sx_award_text"><h1>五等奖</h1>
             <p><span class="sx_orangeColor">电影票</span><br />带上您的宝贝儿，你懂得！！</p></div>
             <div class="clear"></div>
             <div class="sx_award_p5"></div>
          </li>
          <li>
             <div class="sx_award"><img src="<%=controller.getStaticPath(request)%>/mobile/images/m_aw6.png" alt="" /></div>
             <div class="sx_award_text"><h1>六等奖</h1>
             <p><span class="sx_orangeColor">360wifi--电脑的精神粮食</span></p></div>
             <div class="clear"></div>
             <div class="sx_award_p6"></div>
          </li>
          <li>
             <div class="sx_award"><img src="<%=controller.getStaticPath(request)%>/mobile/images/m_aw7.png" alt="" /></div>
             <div class="sx_award_text"><h1>七等奖</h1>
             <p><span class="sx_orangeColor">20元话费</span><br />今晚够话费打给女友了</p></div>
             <div class="clear"></div>
             <div class="sx_award_p7"></div>
          </li>
          <li>
             <div class="sx_award"><img src="<%=controller.getStaticPath(request)%>/mobile/images/m_aw8.png" alt="" /></div>
             <div class="sx_award_text"><h1>八等奖</h1>
             <p><span class="sx_orangeColor">10元话费</span><br />您的余额已不足十元</p></div>
             <div class="clear"></div>
             <div class="sx_award_p8"></div>
          </li>
          <li>
             <div class="sx_award"><img src="<%=controller.getStaticPath(request)%>/mobile/images/m_aw9.png" alt="" /></div>
             <div class="sx_award_text"><h1>参与奖</h1>
             <p><span class="sx_orangeColor">20元红包抵消券</span><br />仅限于服务手续费</p></div>
             <div class="clear"></div>
             <div class="sx_award_p9"></div>
          </li>
        </ul>
        <div class="clear"></div>
    </div>
    <div class="sx_grey_reg">
           <div class="sx_grey_outline">
           <%
          
            int userCount =userManage.countAccount(); %>
           <p>目前已有<span class="sx_redColor sx_font1"> <%=userCount %></span> 位小伙伴参与活动<span class="sx_orangeColor sx_font1"><a href="#" onclick="mgstga();" class="toReg orangeLink">点击参与注册</a></p>
           <div class="sx_m_person2"></div>
           </div>
    </div>
    <div class="sx_m_block3">
       <h2 class="sx_orangeColor ">龟兔赛跑之<span class="sx_font2">年化收益</span></h2>
       <p class="sx_font1 sx_text2">龟：1%-3 % <br />兔：8%-15%</p>
       <div><p class="sx_img"><img src="<%=controller.getStaticPath(request)%>/mobile/images/m_sx1.jpg" alt="" /></p><br /></div>
       <div style="background:#acc5db;"><p class="sx_img"><img src="<%=controller.getStaticPath(request)%>/mobile/images/m_sx2.jpg" alt="" /></p></div>
       <div><p class="sx_img"><img src="<%=controller.getStaticPath(request)%>/mobile/images/m_sx3.jpg" alt="" /></p></div>
       <div><p class="sx_img"><img src="<%=controller.getStaticPath(request)%>/mobile/images/m_sx4.jpg" alt="" /></p></div>
       <div style="background:#fff;"><p class="sx_img" style="margin-bottom:20px;"><img src="<%=controller.getStaticPath(request)%>/mobile/images/m_sx5web.jpg" alt="" /></p></div>
       <div><p class="sx_img"><img src="<%=controller.getStaticPath(request)%>/mobile/images/m_sx6.jpg" alt="" /></p></div>
       <div><p class="sx_img"><img src="<%=controller.getStaticPath(request)%>/mobile/images/m_sx7.jpg" alt="" /></p></div>
    </div>
    <div class="sx_m_block4" id="sx_reg">
        <div class="sx_m_person3"></div>
        <ul>
          <li class="sx_login_liend"><input type="button" value="注&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;册" onclick="mgstga();" class="sx_orangeBt"/></li>
      </ul>
      <div class="clear"></div>
