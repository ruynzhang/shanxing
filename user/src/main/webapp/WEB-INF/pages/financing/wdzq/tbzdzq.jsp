...
URLParameter parameter = new URLParameter(request,controller.getViewURI(request, com.dimeng.p2p.user.servlets.financing.wdzq.Tbzdzq.class),multiSelect,SORT);
   		...
   		<div class="about">
            ...
            <!--理财管理 S-->
			  ...
			  投标状态： <a class="butStyle no" avalue="huikuan" href="/user/financing/wdzq/hszdzq.html">回款中</a>
				     <a class="butStyle " avalue="toubiaozhong">投标中</a>
         		...
         		<div>
   	    		<div class="newsbox">
                  	<div class="til clearfix Men_bt2">
               		...
               		<form action="<%=controller.getViewURI(request, com.dimeng.p2p.user.servlets.financing.wdzq.Tbzdzq.class)%>" method="post">
		                  ...
		                  <div class="no_table user_bolr bot">
		                    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="user_table tc tableOdd">
		                      <tr class="user_lsbg tb_th">
		                        <th>项目名称<a href="/user/financing/wdzq/tbzdzq.html?s=1"><span class="<%=sort==1?"tip-down":"tip-up"%>"></span></a></th>
		                        <th>开投时间<a href="/user/financing/wdzq/tbzdzq.html?s=2"><span class="<%=sort==2?"tip-down":"tip-up"%>"></span></a></th>
		                        <th>截止时间</th>
		                        <th>投资金额（元）<a href="/user/financing/wdzq/tbzdzq.html?s=3"><span class="<%=sort==3?"tip-down":"tip-up"%>"></span></a></th>
		                        <th>项目收益（元）</th>
		                      </tr>
		                       <%
		                       PagingResult<Tbzdzq> list=service.getLoanAssests(sort,paging);
						     ...
						     %>
		                      <tr>
		                        ...
		                        <a class="blue" href="<%configureProvider.format(out, URLVariable.FINANCING_SBTZ_XQ); %><%=vo.F15+rewriter.getViewSuffix()%>"><%=vo.F16%></a>
		                        <%}else{out.print(vo.F16);}%>
		                        </td>
		                        <td><%=T6230_F13.F==vo.isSxbao?Formater.formatDate(vo.F10):"无"%></td>
		                        <td><%=T6230_F13.F==vo.isSxbao?Formater.formatDate(BdUtil.getTime(vo.F10, vo.F08, vo.F22)):"无"%></td>
		                        <td>￥<%=Formater.formatAmount(vo.F13)%></td>
		                        <td>￥<%=Formater.formatAmount(vo.proSj)%></td>
		                      </tr>
		                      <%}}%>
		                    </table>
		            	  </div>
		                    <div class="page">
								<%AbstractFinancingServlet.rendPaging(out, list,controller.getPagingURI(request,com.dimeng.p2p.user.servlets.financing.wdzq.Tbzdzq.class),parameter.getQueryString()); %>
							</div>
                  </div>         
                  </form>      
                  <div class="clear"></div>
