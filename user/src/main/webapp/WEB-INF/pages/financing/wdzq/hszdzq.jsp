...
URLParameter parameter = new URLParameter(request,controller.getViewURI(request, com.dimeng.p2p.user.servlets.financing.wdzq.Hszdzq.class),multiSelect,SORT);
	...
	<div class="contain clearfix">
		<div class="user_top"></div>
   		<div class="about">
            ...
            <!--理财管理 S-->
			  <div class="tb-state">
			  投标状态： <a class="butStyle" avalue="huikuan">回款中</a>
         		...
         		<div>
   	    		<div class="newsbox">
                  	<div class="til clearfix Men_bt2">
               		...
               		<form action="<%=controller.getViewURI(request, com.dimeng.p2p.user.servlets.financing.wdzq.Hszdzq.class)%>" method="post">
		                  ...
		                  <div class="no_table user_bolr bot">
		                    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="user_table tc tableOdd">
		                      <tr class="user_lsbg tb_th">
		                        <th>项目名称<a href="/user/financing/wdzq/hszdzq.html?s=1"><span class="<%=sort==1?"tip-down":"tip-up"%>"></span></a></th>
		                        <th>起息日<a href="/user/financing/wdzq/hszdzq.html?s=2"><span class="<%=sort==2?"tip-down":"tip-up"%>"></span></a></th>
		                        <th>到期还款日<a href="/user/financing/wdzq/hszdzq.html?s=3"><span class="<%=sort==3?"tip-down":"tip-up"%>"></span></a></th>
		                        <th>投资金额（元）<a href="/user/financing/wdzq/hszdzq.html?s=4"><span class="<%=sort==4?"tip-down":"tip-up"%>"></span></a></th>
		                        <th>项目收益（元）</th>
		                        <th>查看详情</th>
		                        
		                      </tr>
		                       <%
		                       PagingResult<ZqxxEntity> list= service.getRecoverAssests(sort,paging);
						     ...
						     %>
		                      <tr>
		                        ...
		                        <td><%=Formater.formatDate(vo.xghkr)%></td>
		                        <td>￥<%=Formater.formatAmount(vo.F04)%></td>
		                        <td>￥<%=Formater.formatAmount(vo.dsbx.subtract(vo.F04))%></td>
		                        <td>
		                        	...
		                        	<%if(vo.isTransfer){%>
		                        	<a class="blue" onclick="Transfer(<%=vo.zqid%>,<%=vo.F06%>)" class="blue" style="cursor: pointer;" id="qxButton">转让</a>
		                        	<%}%>
		                        </td>
		                      </tr>
		                      <%}}%>
		                    ...
		                    <div class="page">
								<%AbstractFinancingServlet.rendPaging(out, list,controller.getPagingURI(request,com.dimeng.p2p.user.servlets.financing.wdzq.Hszdzq.class),parameter.getQueryString()); %>
							</div>
