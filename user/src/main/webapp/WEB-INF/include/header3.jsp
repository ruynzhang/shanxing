...
<div class="bgBlock1">
    <div id="sx_topbox">
	   ...
	   <%if (dimengSession == null || !dimengSession.isAuthenticated()) {%>
            ...
            <%} else { %>
					<a title="<%=serviceSession.getService(UserManage.class).getAccountName()%>">您好：<%StringHelper.truncation(new HTMLFilter(out), serviceSession.getService(UserManage.class).getAccountName(), 12);%></a>
                ...
                <a href="<%configureProvider.format(out, URLVariable.USER_LETTER);%>">站内信(<span class="red"><%=count %></span>)</a>
					<a href="<%=controller.getStaticPath(request)%>/loginoutDiscuz.html">安全退出</a>
				<%}%></div>
			<div class="sx_share">
			<div class="share">
